...
output_directory = f"reports/bookable_venues/"
...
if os.path.isdir(input_directory):
    ...
    with open(f"{output_directory}/{year}.html", "w") as of:
        ...
        of.write(
            ...
            <head>
                <meta charset='utf-8'/>
                <title>Bookable events at London Open House {year} as of {now:%a %d %B at %H:%M}</title>
                <style type="text/css">
                    body {{
            ...
            <body>
            <h1>Bookable events at London Open House {year}</h1>
            <h3>As of {now:%a %d %B at %H:%M}</h3>
        """
        )
